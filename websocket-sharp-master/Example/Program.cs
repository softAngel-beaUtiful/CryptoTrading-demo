...
namespace Example
{
    ...
    public class Program
  {
    public static void Main (string[] args)
    {
      ...
      //using (var ws = new WebSocket ("ws://47.75.172.146:10109/PingPong"))
      ...
      using (var ws = new WebSocket("ws://47.75.172.146:10109/Subscribe")) 
      {
                // Set the WebSocket events.

                Dictionary<string, string> dd = new Dictionary<string, string>(); //cc = new Dictionary<string, string>(), ee = new Dictionary<string, string>(); ;
                dd.Add("Okex", "eth_btc, eth_usdt, btc_usdt");
                dd.Add("Binance", "ETHBTC, DASHETH, BTCUSDT");
                dd.Add("Huobi", "btcusdt, ethbtc, ethusdt");
                ClientRequest cr = new ClientRequest() { OP = "Subscribe", Params = new Dictionary<string, string>[] { dd } };
                string clientrequest = JsonConvert.SerializeObject(cr);
                ws.OnOpen += (sender, e) => ws.Send (clientrequest);
