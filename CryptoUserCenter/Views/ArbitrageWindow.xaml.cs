...
namespace CryptoUserCenter.Views
{
    ...
    public partial class ArbitrageWindow : Window
    {
        ...
        decimal HIGHEST;
        decimal LOWEST;
        public Models.ChartViewModel chartVM;
        public List<OrderBook> ArbDataList { get; set; }
        public ObservableCollection<double[]> IndicatorCollection { get; set; }
        public (string, string) symbolspair = ("bitmex_xbtusd", "okex_btc_usd_220624");
        public ArbitrageWindow(MainWindow mainwin)
        {
            Owner = mainwin;
            InitializeComponent();
            chartVM = new Models.ChartViewModel(this, symbolspair);
        ...
        public static readonly DependencyProperty UpdateByProperty = DependencyProperty.Register(
            "UpdateBy", typeof(object), typeof(ArbitrageWindow),
            new PropertyMetadata(null, new PropertyChangedCallback((d, arg) => {
                var me = d as ArbitrageWindow;
                //Do something
                if (me.DataSource == null)
        ...
        */
        public void UpdateChart(Size newSize, bool force)
        {
            ...
            int num = Math.Min(avail, DataSource.Count);
            Startpoint = DataSource.Count - num;
            HIGHEST = ((OrderBook)DataSource[Startpoint]).AskPrice1;
            LOWEST = ((OrderBook)DataSource[Startpoint]).BidPrice1;
            ...
            foreach (var graph in _graphs)
            {
                ...
                {
                    var high = ((OrderBook)DataSource[Startpoint]).AskPrice1;
                    var low = ((OrderBook)DataSource[Startpoint]).BidPrice1;
                    if (high > HIGHEST)
                        HIGHEST = high;
                    if (low < LOWEST)
                        LOWEST = low;
                }
                graph.Render((double)LOWEST, (double)HIGHEST);
            }
        }
